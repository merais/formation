...
du projet Sport Data Solution selon l'architecture Medallion :
  ...
  - meta     : suivi du pipeline (hachages fichiers sources, historique des runs)

Gestion des droits PostgreSQL :
  - role_pipeline  : lecture/ecriture raw + staging + gold + meta (pipeline ETL)
...
-- Schema meta : suivi du pipeline (hachages fichiers, historique des runs)
...
CREATE TABLE IF NOT EXISTS raw.activites_strava (
    ...
    commentaire     TEXT,
    -- Timestamp d insertion : permet la detection incrementale des nouvelles lignes
...
-- Employes : 8 colonnes anonymisees + flag actif pour le soft-delete
CREATE TABLE IF NOT EXISTS staging.employes (
    ...
    -- Soft-delete : FALSE si l employe a ete retire du fichier XLSX source
...
# ---------------------------------------------------------------------------
# Definition SQL des tables – schema meta
# Suivi de l etat du pipeline : hachages fichiers sources + historique des runs
# ---------------------------------------------------------------------------

SQL_CREATE_META_TABLES = """
-- Etat des fichiers sources et watermarks du pipeline
-- file_hash    : SHA-256 du fichier XLSX (NULL pour les lignes watermark)
-- watermark    : timestamp de la derniere activite Strava traitee (NULL pour les lignes fichier)
...
-- Historique des executions du pipeline Kestra
CREATE TABLE IF NOT EXISTS meta.pipeline_runs (
    run_id                  SERIAL          PRIMARY KEY,
    run_at                  TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    status                  VARCHAR(20)     NOT NULL,
    rh_changed              BOOLEAN         NOT NULL DEFAULT FALSE,
    sport_changed           BOOLEAN         NOT NULL DEFAULT FALSE,
    new_activities_count    INTEGER         NOT NULL DEFAULT 0
);
"""

# Migration idempotente : ajout des nouvelles colonnes sur base existante
SQL_MIGRATE_COLUMNS = """
ALTER TABLE raw.activites_strava
    ADD COLUMN IF NOT EXISTS inserted_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
ALTER TABLE staging.employes
    ADD COLUMN IF NOT EXISTS actif BOOLEAN NOT NULL DEFAULT TRUE;
-- Migration de meta.pipeline_state : ajout des colonnes watermark et rend file_hash nullable
ALTER TABLE meta.pipeline_state
    ADD COLUMN IF NOT EXISTS watermark TIMESTAMPTZ NULL;
ALTER TABLE meta.pipeline_state
...
# Tables attendues apres une initialisation complete (10 tables)
TABLES_ATTENDUES = {
...
NB_TABLES = len(TABLES_ATTENDUES)  # 10
...
def database_exists(cursor):
    ...
    cursor.execute("""
        ...
        WHERE table_schema IN ('raw', 'staging', 'gold', 'rh_prive', 'meta')
...
def create_schemas(cursor):
    """Cree les schemas raw, staging, gold, rh_prive et meta."""
    logger.info("Creation des schemas (raw, staging, gold, rh_prive, meta)...")
    cursor.execute(SQL_CREATE_SCHEMAS)
    logger.info("Schemas crees avec succes.")


def create_tables(cursor):
    """Cree toutes les tables dans les cinq schemas."""
    ...
    logger.info("Creation des tables du schema meta (pipeline state + runs)...")
    cursor.execute(SQL_CREATE_META_TABLES)

    # Migration idempotente : ajout des colonnes inserted_at et actif
    logger.info("Migration idempotente des colonnes (inserted_at, actif)...")
...
def build_roles_sql():
    ...
    -- role_pipeline : lecture/ecriture meta (watermark, pipeline state)
    GRANT USAGE ON SCHEMA meta TO role_pipeline;
    GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA meta TO role_pipeline;
    GRANT USAGE ON ALL SEQUENCES IN SCHEMA meta TO role_pipeline;
