...
# Forcer l'affichage en temps réel même dans un pipe
...
def load_embeddings() -> np.ndarray:
    ...
    print("  (peut prendre quelques secondes selon la taille du fichier...)")
    sys.stdout.flush()
...
def create_faiss_index(embeddings: np.ndarray) -> faiss.Index:
    ...
    print(f"\nAjout des vecteurs à l'index par batches...")
    sys.stdout.flush()
    batch_size = 1000
    for i in range(0, n_vectors, batch_size):
        end = min(i + batch_size, n_vectors)
        index.add(embeddings[i:end])
        pct = int(end / n_vectors * 100)
        print(f"  {end:,}/{n_vectors:,} vecteurs ajoutés ({pct}%)")
        sys.stdout.flush()
