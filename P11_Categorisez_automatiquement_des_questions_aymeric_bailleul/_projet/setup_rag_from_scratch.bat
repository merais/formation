...
if not exist ".venv\Scripts\python.exe" (
    ...
    if /i "!INSTALL_ENV!"=="O" (
        ...
        if errorlevel 1 (
            ...
            echo Conseil : relancez en activant les logs : poetry install --no-root --extras dev -v
            echo.
            pause
            exit /b 1
        )
        ...
        echo   3. Executez : poetry install --no-root --extras dev
...
set "GDRIVE_ID=1wDLjemDVFWpNlzIrHG1xWc-VDUPfK1_j"
set "DOWNLOAD_URL=https://drive.usercontent.google.com/download?id=1wDLjemDVFWpNlzIrHG1xWc-VDUPfK1_j&export=download&confirm=t"
set "MIN_SIZE_BYTES=1070000000"
...
if !NEED_DOWNLOAD! EQU 1 (
    echo       Dataset absent ou corrompu. Telechargement depuis Google Drive...
    echo       Source : https://drive.google.com/file/d/!GDRIVE_ID!/view
    ...
    if not errorlevel 1 (
        curl.exe -L -C - --retry 5 --retry-delay 10 --progress-bar -o "%RAW_FILE%" "!DOWNLOAD_URL!"
    ) else (
...
:: Initialiser le log en UTF-8 sans BOM via .NET (evite le melange d'encodages)
powershell -NoProfile -Command "[System.IO.File]::WriteAllText('!LOG_FILE!', \"============================================================`r`n SETUP RAG FROM SCRATCH  -  %date% %time%`r`n============================================================`r`n\", [System.Text.UTF8Encoding]::new($false))"

:: Lancement du pipeline - StreamWriter UTF-8 pour un encodage coherent du debut a la fin
.venv\Scripts\python.exe -u src\build_vectorstore.py 2>&1 | powershell -NoProfile -Command "$w = [System.IO.StreamWriter]::new('!LOG_FILE!', $true, [System.Text.UTF8Encoding]::new($false)); try { $input | ForEach-Object { Write-Host $_; $w.WriteLine($_) } } finally { $w.Close() }"
